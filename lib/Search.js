...
class Search {
  ...
  constructor(robot) {
    this.robot = robot;
    //this.teamId = '111';//robot.user.id // TODO: teamId ???
  ...
  respond(response) {
    ...
    self.robot.on('loggedIn', (user, team) => {
      console.log('LoggedIn:', arguments);
      self.teamId = team.id;
    });

    self.store = new StorageService();

    if (!query) {
      response.reply('Try typing what you are looking for. Eg. `search burger` :hamburger:')
      return;
    }
  ...
  _handleFoundRestaurantsDialog(response, restaurants) {
    ...
    dialog.addChoice(/(\d+)+\D*/g, (choicesRes) => {
      ...
      if (choices.length) {
        ...
        self.store.addTeamPlaces(self.teamId, places.map(p => p.id));
        self.store.destroy();
