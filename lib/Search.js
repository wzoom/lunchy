...
class Search {
  ...
  removePlace(response) {
    ...
    Promise.all([
      ...
      .then(({foundRestaurants, teamPlaceIds}) => {
        if (foundRestaurants && foundRestaurants.length) {
          return foundRestaurants.reduce((restaurant, pairs) => {
            // Only restaurants that we have stored in Redis
            if (!teamPlaceIds.contains(restaurant.id)) return pairs;
            return pairs[restaurant.id] = restaurant.name
          }, {})
        } else {
          return {}
        }
      })
      .then(({namesByIdMap, teamPlaceIds}) => {
        console.log('Pairs:', namesByIdMap);
