...
class Search {
  ...
  respond(response) {
    ...
    const team = response.message.room === 'Shell' ? 'Shell' : response.message.team;
    if (!team) {
      console.log('Cannot find TeamId ("team") in Response Message:', JSON.stringify(response.message));
