    ...
    return this._getCached('/dailymenu' + this._query({
      ...
      if (_.isEmpty(obj.daily_menus)) {
        const emptyMenuRestaurant = {
          ...
          dishes: [{
            name: 'No Daily Menu Available! :knife_fork_plate:',
            price: ''
          }]
        };
        return self.getRestaurant(restaurantId).then(
          (restaurant) => Object.assign({}, emptyMenuRestaurant, {name: restaurant.name}),
          (err) => emptyMenuRestaurant
        );
        //throw new Error('There is no daily_menu in the response for restaurantId ' + restaurantId);
      }

      let first_menu = obj.daily_menus[0].daily_menu;
      console.log('Restaurant', restaurantId, 'Menu:', JSON.stringify(first_menu));
      return first_menu;
    });
  }
