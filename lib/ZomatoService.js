...
class ZomatoService {
  ...
  /**
   * Closure that returns recursive function to fetch maxCount restaurants by calling GET recursively.
   * TODO: Rewrite recursion to simple iteration + Promise.all(fetch10-20, fetch20-30, fetch30-40, ...)
   *
   * @param maxCount
   ...
   * @private
   */
  _fetchMaxCount(maxCount, originalQueryParams = {}) {
    let self = this;

    return ({results_found, results_start, results_shown, restaurants}) => {

      if (maxCount > results_start + results_shown) {
        const params = Object.assign({}, originalQueryParams, {
          count: results_shown,
          start: results_start
        });

        return self._search(params)
          .then(self._fetchMaxCount(maxCount, originalQueryParams))
          .then((fetchedRestaurants) => [...restaurants, ...fetchedRestaurants]);
      }
  ...
  search(query, maxCount = 10, locationId = this.DEFAULT_LOCATION_ID) {
    const params = {
      q: query,
    ...
    return this._search(params).then(this._fetchMaxCount(maxCount, params));
  }
